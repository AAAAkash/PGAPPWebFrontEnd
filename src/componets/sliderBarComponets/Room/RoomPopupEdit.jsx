...
const RoomPopupEdit = ({ onClose, isRoom , fetchData}) => {
   const [user, setUser] = useState(isRoom || {
    BuildingId: "",
    Room_No: "",
    Beds: "",
  });
  ...
  const validateForm = () => {
    ...
    if (!user.Room_No) {
      ...
      validationErrors.Room_No = "Room Number must be a number";
    }
    if (!user.Beds) {
    ...
    if (Object.keys(validationErrors).length === 0) {
      try {
        const response = await fetch(
          dbConfig.backendBaseUrl + `/main/RoomsData/UpdateRoom/${user._id}`,
          {
            method: "PUT",
            headers: {
        ...
        if (response.ok) {
          // console.log(
          //   "Room data Edit successfully:",
          //   responseData.BuildingData
          // );
          alert("Room Edit Successfully")
          onClose();
          fetchData();
        } else {
        ...
        console.error("Error Edit Room data:", error);
      }
    }
  };
