...
import PopupView from "./HistoryOne"
...
const ManagerPaymentHistory = () => {
  ...
  // console.log(studentData,"22222222");
  ...
  // console.log(buildingData,"3333333333333");
  ...
  const fetchData = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setStudentData(fetchedData.students);
      } else {
  ...
  const handleOneView = (id) => {
    // Find the selected company based on the ID
    const studentDataView = studentData.find((company) => company._id === id);
    setViewOneData(studentDataView);
  ...
  const toggleFilterInput = () => {
    ...
    if (isFilterOpen) {
      setUser(prevUser => ({
        ...prevUser,
        assignedBuilding: tempBuildingId,
        Room_No: tempRoomId,  
        Date: selectedTime          
      }));
      setTempBuildingId("");
  ...
  const buildingsToEdit = roomData.filter((company) => company.BuildingId === tempBuildingId || tempBuildingId.length === 0? company.BuildingId: "");
  const uniqueAvailableBeds = buildingsToEdit.flatMap(room => [{ Id: room._id, Value: room.Room_No }]);
        ...
        <div className="second-Div">
          ...
          <div className="h-scree three">
            ...
            <div className="NAMEandBUTTON">
              ...
              <div className="d-flex flex-row grid gap-4 ">
        ...
        <div className="position-relative">
            <div className="compailntbuttonbuilding">           
              <div className="ComplainPopupBTNDIVFilter ">
                <div className="filterDivName" onClick={toggleFilterInput}>
                  <h6>Filter</h6>
                  <div className="mt-0.4"><IoIosArrowDown /></div>
                </div>
              </div>

              <div className="SEARCHDIV">
                <svg width="17" height="17" viewBox="0 0 20 19" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path fillRule="evenodd" clipRule="evenodd" d="M0.958313 9.0835C0.958313 4.32004 4.81986 0.458496 9.58331 0.458496C14.3468 0.458496 18.2083 4.32004 18.2083 9.0835C18.2083 11.12 17.5025 12.9916 16.3222 14.4671L19.5636 17.1449L18.2083 18.5002L14.9669 15.8224C13.4914 17.0027 11.6198 17.7085 9.58331 17.7085C4.81986 17.7085 0.958313 13.847 0.958313 9.0835ZM9.58331 2.37516C5.8784 2.37516 2.87498 5.37859 2.87498 9.0835C2.87498 12.7884 5.8784 15.7918 9.58331 15.7918C13.2882 15.7918 16.2916 12.7884 16.2916 9.0835C16.2916 5.37859 13.2882 2.37516 9.58331 2.37516Z" fill="#8C8C8C" />
                </svg>


                <input type="search" placeholder="Search " onChange={handleSearch} value={searchQuery}/>


              </div>
            </div>
            {isFilterOpen ? (
            <div className="filterInputMainDiv position-absolute">
                      <h4>Filter</h4>

                      <div className="filterInput">
                        <label htmlFor="text">Select Building</label>
                        <select
                            name="assignedBuilding"
                      ...
                      <div className="filterInput">
                        <label htmlFor="text">Select Room No.</label>
                        <select
                          name="Room_No"
                        ...
                        {uniqueAvailableBeds.length === 0 ? (
                          <option value="" disabled>No Room in this Building</option>
                        ) : (
                      ...
                      <div>
                        <button onClick={toggleFilterInput}>Apply</button>
                      </div>
            </div >
            ):null}
            ...
            <div className="four">
              ...
              {studentData && studentData.length > 0 ? (
                    ...
                    .filter((item) => {
                      ...
                      const phoneNumberString = String(item.phoneNumber); // Convert PhoneNumber to string
                      return (
                        item.studentName
                          .toLowerCase()
                    ...
                    .filter((item) => {
                      // Conditionally apply the filter based on the value of buildingId
                      if (buildingId === "" || buildingId === null) {
                        return true; // Return true to include all data
                      } else {
                        return item.assignedBuilding === buildingId // Filter based on assignedBuilding matching buildingId
                      }
                    })
                    .filter((item) => {
                      // Conditionally apply the filter based on the value of buildingId
                      if (user.assignedBuilding === "" || user.assignedBuilding === null) {
                        return true; // Return true to include all data
                      } else {
                        return item.assignedBuilding === user.assignedBuilding // Filter based on assignedBuilding matching buildingId
                      }
                    })
                    .filter((item) => {
                      // Conditionally apply the filter based on the value of buildingId
                      if (user.Room_No === "" || user.Room_No === null) {
                        return true; // Return true to include all data
                      } else {
                        return item.RoomDetails.Room_No === user.Room_No // Filter based on assignedBuilding matching buildingId
                      }
                    })
                    .filter((item) => {
                      const month = new Date(item.createdAt).getMonth();
                      // console.log(month,"22222222");
                      // Conditionally apply the filter based on the value of buildingId
                      if (user.Date === "" || user.Date === null) {
                        return true; // Return true to include all data
                      } else {
                        return month === user.Date; // Filter based on assignedBuilding matching buildingId
                      }
                    })
                    .map((filteredItem, index) => (
                      <div
                        key={index}
                        className="CAshALLDATa"  
                        style={{cursor:"pointer"}} 
                        onClick={() => handleOneView(filteredItem._id)}
                      >
                        ...
                        <div className="contant_Text">
                          ...
                          <h6>Mob No : {filteredItem.phoneNumber}</h6>
                          <h6>Email ID: {filteredItem.email}</h6>
                          <h6>Building Assigned: {filteredItem.BuildingDetails.Building}</h6>
                          {/* <div className="contant_TextRECEIVED grid gap-1">
                            <h6>Payment Status :</h6>
                            <p className="" style={{ color: filteredItem.paymentStatus === 'Pending' ? '#F94242' : '#50AB41' }}>
                              {filteredItem.paymentStatus}
                            </p>


                          </div>
                          {filteredItem.paymentStatus === 'Pending' && (
                              ...
                              <p>Send Reminder</p>
                               ...
                               <div className="contant_TextRECEIVED grid gap-1">
                                ...
                                <p
                                  ...
                                  style={{
                                    color:
                                      filteredItem.paymentStatus === "Pending" || filteredItem.CashTaken === "Manager"
                                        ? "#F94242"
                                        : "#50AB41",
                                  }}
                                >
                                  {filteredItem.CashTaken === "Manager" ? "Pending" : filteredItem.paymentStatus}
                                </p>
                              </div>
                              {filteredItem.paymentStatus === "Pending" || filteredItem.CashTaken === "Manager" ? (
                                <div className="CHECKBOXofStudent mt-2 ">
...
//       key: '1',
//       label: (      
//         <div className="filterInputMainDiv" >
//               <h4>Filter</h4>
...
//               <div className="filterInput">
//                 <label htmlFor="text">Select Time</label>
//                 <select> <option> 3 Months</option> </select>           
//               </div>
...
// {getdata
//   .filter((item) =>
//     item.ManagerName.includes("65ba3f0c4408b060da9afcd7")
//   )
//   .map((filteredItem, index) => (
//     <>
//       <div className="flex">
//         <h1 key={index} style={{ paddingRight: "20px" }}>
//           {filteredItem.Building}
//         </h1>
//         {filteredItem.getMangers?.map(
//           (manager, index, array) => (
//             <span key={manager.id}>
//               {""} {manager.ManagerName}
...
// console.log(getdata,"22222222222222");
...
//         // console.log(filteredData, "buildings11111111111");
//       } else {
//         console.error("Failed to fetch data:", response.status);
//       }
//     } catch (error) {
