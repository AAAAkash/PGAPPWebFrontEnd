...
const ManagerManagementPopupBlock = ({ onClose, BlockManager, fetchData }) => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await fetch(
        dbConfig.backendBaseUrl + `/main/AdminManager/Managerblock/${BlockManager._id}`,
      ...
      if (response.ok) {
        ...
        console.error("Failed to block Manager:", response.statusText);
      }
    } catch (error) {
      console.error("Error blocking Manager:", error.message);
    }
  };
...
export default ManagerManagementPopupBlock;
